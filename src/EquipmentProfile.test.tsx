...
it('offers rear camera and file selection and shows the saved photo for the current equipment', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Chụp ảnh', exact: true })).toBeVisible()
  expect(screen.getByRole('button', { name: 'Tải ảnh lên', exact: true })).toBeVisible()
...
it('keeps retry available when upload fails without claiming a saved photo', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Chụp ảnh', exact: true })).toBeEnabled()
})
