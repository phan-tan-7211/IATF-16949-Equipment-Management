...
const NAV: Array<{ id: View; label: string; adminOnly?: boolean }> = [
  { id: 'dashboard', label: 'Tổng quan' }, { id: 'qr', label: 'Quét QR' }, { id: 'equipment', label: 'Thiết bị' }, { id: 'inspection', label: 'Kiểm tra ngày' }, { id: 'maintenance', label: 'Bảo trì' }, { id: 'tooling', label: 'Jig & Tooling' }, { id: 'calibration', label: 'Hiệu chuẩn' }, { id: 'settings', label: 'Audit & Cấu hình', adminOnly: true },
]

function initialView(): View { const requested=new URLSearchParams(window.location.search).get('phase3'); if(requested==='qr'||requested==='equipment'||requested==='dashboard'||requested==='inspection'||requested==='maintenance'||requested==='tooling'||requested==='calibration')return requested;if(requested==='audit')return'settings';return'dashboard' }
...
function LiveView({view,equipmentTarget,onOpenEquipment,onCloseQrResult,onEditQrResult}:{view:View;equipmentTarget:string;onOpenEquipment:(equipmentId:string)=>void;onCloseQrResult:()=>void;onEditQrResult:()=>void}){
  ...
  if(view==='calibration')return <div className="maintenance-workspace-stack"><LiveCalibrationPanel/><LiveCalibrationEvaluationPanel/></div>
  return <LiveAuditPanel />
}

export default function App(){
  const[view,setView]=useState<View>(initialView);const[equipmentTarget,setEquipmentTarget]=useState(initialEquipmentTarget);const[role,setRole]=useState<AppRole>('UNKNOWN');const[sessionEmail,setSessionEmail]=useState('');const[roleLoaded,setRoleLoaded]=useState(false)
  useEffect(()=>{let active=true;void loadLiveSession().then((session)=>{if(!active)return;setRole(normalizeRole(session.role));setSessionEmail(session.email)}).catch(()=>{if(!active)return;setRole('UNKNOWN');setSessionEmail('')}).finally(()=>{if(active)setRoleLoaded(true)});return()=>{active=false}},[])
  useEffect(()=>{if(roleLoaded&&view==='settings'&&!canViewAudit(role))setView('dashboard')},[role,roleLoaded,view])
  const visibleNav=useMemo(()=>NAV.filter((item)=>!item.adminOnly||canViewAudit(role)),[role]);const active=useMemo(()=>NAV.find((item)=>item.id===view)??NAV[0],[view])
  function syncUrl(nextView:View,equipmentId=''){const url=new URL(window.location.href);url.searchParams.set('phase3',nextView==='settings'?'audit':nextView);if(equipmentId)url.searchParams.set('equipment',equipmentId);else url.searchParams.delete('equipment');window.history.replaceState({},'',url)}
