...
type View = 'dashboard' | 'qr' | 'equipment' | 'inventory' | 'inspection' | 'maintenance' | 'spare' | 'tooling' | 'calibration' | 'print' | 'settings'
...
function initialView():View{const requested=new URLSearchParams(window.location.search).get('phase3');if(requested==='qr'||requested==='equipment'||requested==='inventory'||requested==='dashboard'||requested==='inspection'||requested==='maintenance'||requested==='spare'||requested==='tooling'||requested==='calibration'||requested==='print')return requested;if(requested==='audit')return'settings';return'dashboard'}
