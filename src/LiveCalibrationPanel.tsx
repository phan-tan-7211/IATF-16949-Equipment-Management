...
export function LiveCalibrationPanel() {
  ...
  useEffect(() => { let active=true; loadLiveCalibration().then((r)=>{if(active){setRows(r);setError('')}}).catch((c:unknown)=>{if(active)setError(c instanceof Error?c.message:'Không thể tải danh mục hiệu chuẩn')}).finally(()=>{if(active)setLoading(false)}); return()=>{active=false} }, [])
  useEffect(() => {
  ...
  return <div className="calibration-page">
    ...
    <section className="calibration-surface" aria-labelledby="calibration-title">
      <header className="calibration-header"><div><p className="eyebrow">CEV-BM-STCL-03</p><h2 id="calibration-title">Kiểm soát hiệu chuẩn</h2><p>{filteredRows.length} / {rows.length} thiết bị · dữ liệu trực tiếp</p></div><button type="button" onClick={()=>void reload()}>Làm mới</button></header>
      ...
      {message?<div className="calibration-state success">{message}</div>:null}{loading?<div className="calibration-state">Đang tải danh mục hiệu chuẩn…</div>:null}{error?<div className="calibration-state error">{error}</div>:null}
