...
export function LiveEquipmentRegistrationPanel() {
  ...
  return <section className="equipment-register-card">
    ...
    {open ? <form className="equipment-register-form" onSubmit={submit}>
      ...
      <label className="wide"><span>Thông số kỹ thuật</span><textarea rows={3} list="master-technicalSpecification-suggestions" value={form.technicalSpecification || ''} onChange={(e) => setForm({ ...form, technicalSpecification: e.target.value })} onBlur={() => setForm((current) => ({ ...current, technicalSpecification: canonicalizeMasterValue(current.technicalSpecification, suggestions.technicalSpecification) }))} /><datalist id="master-technicalSpecification-suggestions">{suggestions.technicalSpecification.map((value) => <option key={value} value={value} />)}</datalist></label>
