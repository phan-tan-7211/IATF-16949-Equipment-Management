...
export function LiveDashboardPanel() {
  ...
  return <div className="stack">
    <section className="hero-card" aria-labelledby="live-dashboard-title">
      <div>
        <p className="eyebrow">IATF 16949 · Production live</p>
        <h2 id="live-dashboard-title">Thiết bị trong tầm kiểm soát</h2>
        <p>Dữ liệu trực tiếp từ Apps Script backend và Google Sheets. Không sử dụng mock trong dashboard live.</p>
      </div>
      <span className="status-pill">LIVE BACKEND</span>
    </section>

    {loading ? <section className="content-card"><p className="muted" role="status">Đang tải dữ liệu production…</p></section> : null}
    {error ? <section className="content-card" role="alert"><b>Không kết nối được backend</b><p>{error}</p></section> : null}
    ...
    {!loading && !error ? <>
      <section className="metric-grid" aria-label="Tổng quan thiết bị live">
        ...
        <article><span>Đang hoạt động</span><strong>{summary.runningCount}</strong><small>Equipment_Master</small></article>
        <article><span>Máy DOWN</span><strong>{summary.downCount}</strong><small>Cần xử lý</small></article>
        <article><span>PM quá hạn</span><strong>{summary.pmOverdue}</strong><small>Maintenance_Plan</small></article>
      </section>
      ...
      <section className="metric-grid" aria-label="Tổng quan nghiệp vụ live">
        ...
        <article><span>Downtime đang mở</span><strong>{summary.downtimeOpen}</strong><small>Chưa restoredAt</small></article>
        <article><span>Downtime ghi nhận</span><strong>{summary.downtimeMinutes}</strong><small>phút</small></article>
      </section>

      <section className="content-card" aria-labelledby="live-flow-title">
        <div className="section-heading"><div><p className="eyebrow">Workflow live</p><h3 id="live-flow-title">Luồng hồ sơ số hóa</h3></div></div>
