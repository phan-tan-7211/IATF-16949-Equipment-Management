...
type PhotoHover = { url: string; name: string; x: number; y: number; size: number }
...
function photoHoverPosition(clientX:number,clientY:number){
  ...
  if(y+size>window.innerHeight-margin)y=clientY-size-gap
...
export function LiveEquipmentPanel() {
  ...
  function openPhotoHover(url:string,name:string,clientX:number,clientY:number){
    ...
    const position=photoHoverPosition(clientX,clientY)
    setPhotoHover({url,name,...position})
  }
