...
import { getEquipmentCacheSnapshot, getEquipmentPhotoPreview, getEquipmentPhotoPreviews, loadSupabaseEquipment, uploadEquipmentPhoto } from './data/supabaseEquipment'
...
export function LiveEquipmentPanel() {
  ...
  const [rows,setRows]=useState<LiveEquipment[]>(()=>getEquipmentCacheSnapshot()); const [photos,setPhotos]=useState<Record<string,PhotoInfo>>({}); const [editing,setEditing]=useState<EquipmentMasterEditInput|null>(null); const [profileId,setProfileId]=useState('')
  ...
  async function refreshOnePhoto(equipmentId:string){ setPhotos((current)=>({...current,[equipmentId]:{state:'loading',url:current[equipmentId]?.url||''}})); try{const preview=await getEquipmentPhotoPreview(equipmentId);setPhotos((current)=>({...current,[equipmentId]:{state:preview.exists?'yes':'no',url:preview.signedUrl}}));return preview.exists}catch{setPhotos((current)=>({...current,[equipmentId]:{state:'error',url:''}}));return false} }
  async function refreshPhotoStates(result:LiveEquipment[]){ setPhotos((current)=>Object.fromEntries(result.map((row)=>[row.equipmentId,current[row.equipmentId]||{state:'loading',url:''} as PhotoInfo]))); try{const previews=await getEquipmentPhotoPreviews(result.map((row)=>row.equipmentId));setPhotos(Object.fromEntries(result.map((row)=>{const preview=previews[row.equipmentId];return [row.equipmentId,{state:preview?.exists?'yes':'no',url:preview?.signedUrl||''} as PhotoInfo]})))}catch{setPhotos((current)=>Object.fromEntries(result.map((row)=>[row.equipmentId,current[row.equipmentId]||{state:'error',url:''} as PhotoInfo])))} }
  ...
  useEffect(()=>{const snapshot=getEquipmentCacheSnapshot();if(snapshot.length){setRows(snapshot);setLoading(false);void refreshPhotoStates(snapshot);void loadSupabaseEquipment({force:true}).then((result)=>{setRows(result);setError('');void refreshPhotoStates(result)}).catch(()=>undefined)}else void reloadEquipment(true)},[])
