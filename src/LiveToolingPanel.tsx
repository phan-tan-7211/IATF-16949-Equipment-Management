...
export function LiveToolingPanel() {
  ...
  return <div className="tooling-page">
    ...
    <section className="tooling-surface">
      <header className="tooling-header"><div><p className="eyebrow">Jig & Tooling Control</p><h2>Tooling Workspace</h2><p>Dữ liệu và workflow trực tiếp từ Supabase RPC</p></div><div className="tooling-actions"><button type="button" onClick={() => void refresh()}>Làm mới</button><button type="button" onClick={() => setDrawer('TOOLING')}>+ Tooling</button><button type="button" onClick={() => setDrawer('PLAN')}>+ Kế hoạch</button><button className="tooling-primary" type="button" onClick={() => setDrawer('MOD')}>+ Modification</button></div></header>
