...
export function LiveSparePartsAutoPanel() {
  ...
  return <div className="spare-page">
    <section className="spare-summary"><article><span>Danh mục</span><strong>{parts.length}</strong><small>linh kiện đã biết</small></article><article><span>Required</span><strong>{required}</strong><small>cần bảo đảm khả dụng</small></article><article className={needBuy?'warn':''}><span>Cần xem xét mua</span><strong>{needBuy}</strong><small>tồn ≤ min</small></article><article><span>Mã tự sinh</span><strong>SP</strong><small>SP-00001…</small></article></section>
    <section className="spare-surface">
      <header className="spare-header"><div><p className="eyebrow">Replacement Parts · IATF 8.5.1.5</p><h2>Linh kiện thay thế</h2><p>TAN chỉ điền flow. Mã quản lý và phân loại rủi ro do hệ thống tự sinh.</p></div>{canEdit?<button className="spare-primary" onClick={startNew}>+ Thêm linh kiện</button>:null}</header>
      <div className="spare-toolbar"><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Tìm mã, Part No., tên, máy, maker…"/><label><input type="checkbox" checked={purchaseOnly} onChange={(e)=>setPurchaseOnly(e.target.checked)}/> Chỉ hiện cần mua</label></div>
      ...
      {!loading?<div className="spare-grid"><div className="spare-list">{filtered.map((part)=><button key={part.partId} className={`spare-card ${selectedId===part.partId?'active':''}`} onClick={()=>{setSelectedId(part.partId);setFormOpen(false);setUsageEquipment('')}}><div><strong>{part.partName}</strong><small>{part.partId}{part.partNumber?` · ${part.partNumber}`:''}</small></div><span className={`spare-badge ${part.classification.toLowerCase()}`}>{LABEL[part.classification]} {part.riskScore}/4</span><div className="spare-card-meta"><span>{part.equipmentCount} máy</span><span>Tồn {part.stockQty} / Min {part.minQty}</span>{part.stockQty<=part.minQty&&part.classification!=='NORMAL'?<b>Cần mua</b>:null}</div></button>)}</div>
        <div className="spare-detail">{formOpen?<form onSubmit={submit} className="spare-form"><header><div><p className="eyebrow">Flow đăng ký</p><h3>{form.partId||'Linh kiện mới'}</h3><p>{form.partId?'Đang sửa mã hiện có':'Mã SP-xxxxx sẽ tự sinh khi lưu'}</p></div><span className={`spare-badge ${preview.classification.toLowerCase()}`}>{LABEL[preview.classification]} {preview.score}/4</span></header>
          ...
          <EquipmentMultiSelect equipment={equipment} selectedIds={form.equipmentIds} onChange={(equipmentIds)=>setForm((current)=>({...current,equipmentIds}))} title="Máy sử dụng" helper="Tìm và check nhiều máy cùng lúc. Mỗi máy hiển thị ảnh, mã, tên và cấp độ." disabled={saving}/>
