...
test('QR fits the viewport and suggests equipment by name before opening', async ({ page }, testInfo) => {
  await openApp(page)
  await clickNav(page, 'Quét QR')
  await expect(page.locator('.topbar h1')).toHaveText('Quét QR')
  await expect(page.locator('.qr-hero')).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'Đăng xuất' })).toHaveCount(0)
  const search = page.getByRole('combobox', { name: 'Tìm mã hoặc tên thiết bị' })
  await expect(search).toBeEnabled()
  const sizes = await page.evaluate(() => ({height: window.innerHeight, width: window.innerWidth,
    scrollHeight: document.documentElement.scrollHeight, scrollWidth: document.documentElement.scrollWidth,
    searchBottom: document.querySelector('.qr-manual-card')!.getBoundingClientRect().bottom,
    navTop: getComputedStyle(document.querySelector('.bottom-nav')!).display === 'none' ? window.innerHeight : document.querySelector('.bottom-nav')!.getBoundingClientRect().top}))
  expect(sizes.scrollHeight).toBeLessThanOrEqual(sizes.height + 1)
  expect(sizes.scrollWidth).toBeLessThanOrEqual(sizes.width + 1)
  expect(sizes.searchBottom).toBeLessThanOrEqual(sizes.navTop)
  await testInfo.attach('QR ready', {body: await page.screenshot(), contentType: 'image/png'})
  await search.fill('Smoke')
  const option = page.getByRole('option', {name: /CEV-PR-001.*Smoke Equipment/})
...
test('account details close with Escape and remain keyboard accessible', async ({ page }) => {
  await openApp(page)
  const account = page.getByRole('button', {name: 'Tài khoản', exact: true})
  await account.click()
  await expect(page.getByRole('region', {name: 'Thông tin tài khoản'})).toContainText('ADMIN')
...
test('equipment dialogs scroll internally and release the background on close', async ({ page }) => {
  ...
  await profile.evaluate(el => { el.scrollTop = 120 })
  await expect.poll(() => profile.evaluate(el => el.scrollTop)).toBeGreaterThan(0)
  ...
  await expect(page.locator('body')).toHaveCSS('position', 'fixed')
  await drawer.evaluate(el => { el.scrollTop = 120 })
  await expect.poll(() => drawer.evaluate(el => el.scrollTop)).toBeGreaterThan(0)
  await expect(drawer).toHaveCSS('overscroll-behavior', 'contain')
