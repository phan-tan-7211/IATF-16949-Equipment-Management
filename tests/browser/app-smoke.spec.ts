...
test('maintenance opens work orders first and can switch record tabs', async ({ page }) => {
  ...
  await expect(intake.getByLabel('Thiết bị *', { exact: true })).toBeVisible()
  await expect(intake.getByLabel('Mức ưu tiên *', { exact: true })).toBeVisible()
  await expect(intake.getByLabel('Lý do / hiện tượng *', { exact: true })).toBeVisible()
  await expect(intake.getByLabel('Xử lý dự kiến / ghi chú tiếp nhận', { exact: true })).toBeVisible()
  await expect(intake.getByLabel('Bắt đầu dự kiến', { exact: true })).toBeVisible()
  await expect(intake.getByLabel('Kết thúc dự kiến', { exact: true })).toBeVisible()
