-- Automatic equipment A/B/C/D classification from facts supplied during registration.
-- Rule version: CEV-ABCD-V1.
-- Backward-compatible with the currently deployed registration UI until the new UI is rolled out.
...
begin
  ...
  v_has_auto_facts := v_stops_production is not null
  ...
  if v_has_auto_facts then
    ...
    -- Temporary compatibility for the already deployed pre-V1 registration UI.
  ...
  insert into public.equipment_master(
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'criticalityFacts',case when v_has_auto_facts then jsonb_build_object(
