-- Equipment management responsibility for ISO/IATF identification labels.
-- Primary responsible person is mandatory for new records and when editing legacy records.
-- Secondary responsible person is optional and may be cleared.
...
begin
  ...
  v_primary:=trim(regexp_replace(coalesce(p_input->>'managementResponsiblePrimary',''),'\s+',' ','g'));
  v_secondary:=trim(regexp_replace(coalesce(p_input->>'managementResponsibleSecondary',''),'\s+',' ','g'));
  if v_name='' then raise exception 'EQUIPMENT_NAME_REQUIRED'; end if;
  if v_primary='' then raise exception 'MANAGEMENT_RESPONSIBLE_PRIMARY_REQUIRED'; end if;
  ...
  values(
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'managementResponsiblePrimary',v_primary,
      'managementResponsibleSecondary',nullif(v_secondary,''),
  ...
  values('AUD-'||to_char(clock_timestamp(),'YYYYMMDDHH24MISSMS')||'-'||substr(replace(gen_random_uuid()::text,'-',''),1,6),v_id,'Equipment',v_id,'CREATE_EQUIPMENT_AUTO_ID',v_actor,jsonb_build_object('equipmentType',v_type,'criticality',v_criticality,'criticalityRule','CEV-ABCD-V2','managementResponsiblePrimary',v_primary,'managementResponsibleSecondary',nullif(v_secondary,'')));
...
begin
  ...
  v_source:=(coalesce(v_old.source_data,'{}'::jsonb) - 'managementResponsiblePrimary' - 'managementResponsibleSecondary') || jsonb_strip_nulls(jsonb_build_object(
    'equipmentCategory',nullif(trim(regexp_replace(coalesce(p_input->>'equipmentCategory',''),'\s+',' ','g')),''),
    'currentArea',nullif(trim(regexp_replace(coalesce(p_input->>'currentArea',''),'\s+',' ','g')),''),
    'currentLine',nullif(trim(regexp_replace(coalesce(p_input->>'currentLine',''),'\s+',' ','g')),''),
    'managingDepartment',nullif(trim(regexp_replace(coalesce(p_input->>'managingDepartment',''),'\s+',' ','g')),''),
