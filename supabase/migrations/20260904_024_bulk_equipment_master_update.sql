-- Safe bulk edits for operational master fields only.
-- Equipment type is intentionally excluded because CEV-PR / CEV-ME IDs encode the type.
...
begin
  ...
  foreach v_id in array p_equipment_ids loop
    ...
    update public.equipment_master set
      ...
      status=case when p_patch ? 'status' then v_status::public.equipment_status else status end,
